...
func NewHTTP(registry discovery.Registry) *GatewayHTTP {
	return &GatewayHTTP{
		registry: registry,
	}
}

func (g *GatewayHTTP) GetAgent(ctx context.Context, id string) (*agent.Agent, error) {
	addrs, err := g.registry.ServiceAdresses(ctx, string(agentServiceName))
	if err != nil {
		return nil, err
	}
